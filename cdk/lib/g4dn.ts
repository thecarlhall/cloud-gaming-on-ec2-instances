...
export class G4DNStack extends BaseEc2Stack {
    ...
    protected getUserdata() {
        ...
        userData.addCommands(
            ...
            '',
            `msiexec.exe /i ${this.props.awsCliUrl}`,
            '',
            ...
            `Invoke-WebRequest -Uri "${this.props.vbaudioUrl}" -OutFile $LocalTempPath\\VbAudio.zip`,
            `Expand-Archive "$LocalTempPath\\VbAudio.zip" -DestinationPath $InstallationFilesFolder\\2_VbAudio`,
            `Invoke-WebRequest -Uri "${this.props.nvfbcUrl}" -OutFile $LocalTempPath\\NvFBCEnable.zip`,
            `Expand-Archive "$LocalTempPath\\NvFBCEnable.zip" -DestinationPath $InstallationFilesFolder\\3_NvFBCEnable`,
            `Invoke-WebRequest -Uri "${this.props.parsecUrl}" -OutFile $InstallationFilesFolder\\4_parsec-windows.exe`,
            ...
            `Invoke-WebRequest -Uri "${this.props.steamUrl}" -OutFile $InstallationFilesFolder\\6_SteamSetup.exe`,
